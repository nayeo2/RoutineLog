...
# 52. Multi-Agent Collaboration Rules

메인 에이전트가 전체 작업을 총괄하고 최종 판단과 사용자 답변을 담당한다.

프로젝트 전용 `luna_worker`는 다음과 같은 독립적인 보조 작업에 사용한다.
...
1. 단순하고 하나의 흐름으로 끝나는 작업은 메인 에이전트가 직접 처리한다.
2. 두 개 이상 서로 독립적인 조사 또는 검증 작업으로 나눌 수 있으면 `luna_worker`를 자동 사용한다.
3. 필요 이상으로 subagent를 생성하지 않고 동시에 최대 4개까지만 사용한다.
4. 같은 파일을 여러 Luna가 동시에 수정하지 않도록 작업 경계를 분리한다.
5. Write-heavy 작업보다 Read-heavy 조사와 검증을 병렬 실행하는 것을 우선한다.
6. 메인 에이전트는 Luna의 결과를 기다린 뒤 근거를 종합하고 최종 판단한다.
7. Luna의 직접 코드 변경은 메인 에이전트가 독립적이고 경계가 명확한 작업으로 명시적으로 맡긴 경우에만 허용한다.

`luna_worker`는 다음 사항을 독자적으로 확정하지 않는다.

```text
Architecture 변경
Database / Entity 관계 변경
API 계약 변경
인증 / 인가 / Security 설계
Transaction 정책
Dependency 또는 기술 스택 변경
여러 Domain에 영향을 주는 Refactoring
최종 사용자 답변
```

이러한 사항은 관련 근거, 불일치, 가능한 선택지와 trade-off를 메인 에이전트에게 반환하고 최종 결정은 메인 에이전트가 수행한다.
